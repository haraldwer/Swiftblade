...
bool ImGui::OpenFileBrowser(const std::string& InPath, FileBrowser::PickType InType, const std::set<std::string>& InExt)
{
    // Create instance if it does not exist
    if (!FileBrowser::Instance)
        FileBrowser::Instance = new FileBrowser(); 
    return FileBrowser::Instance->OpenInternal(InPath, InType, InExt);     
}
...
bool ImGui::FileBrowser::OpenInternal(const std::string& InPath, PickType InType, const std::set<std::string>& InExt)
{
    if (IsOpen)
...
void ImGui::FileBrowser::Refresh()
{
    ...
    for (auto& entry : std::filesystem::directory_iterator(path))
    {
        ...
        else if (Type != PickType::DIRECTORY)
        {
            // Filter extensions
            if (!Ext.empty())
            {
