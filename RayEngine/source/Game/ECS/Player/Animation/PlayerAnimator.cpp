...
void ECS::PlayerAnimator::TryOverrideState(const Type& InAnimState) const
{
    if (stateMachine)
...
Mat4F ECS::PlayerAnimator::ToCameraSpace(const Mat4F& InMat, float InWeight) const
{
...
HandState ECS::PlayerAnimator::Flip(const HandState& InHand)
{
...
Vec2F ECS::PlayerAnimator::HeadBob(const float InScale, const float InFrequency, float InHeight, const float InLeaning) const
{
...
Mat4F ECS::PlayerAnimator::HandBob(Mat4F InTrans, Vec2F InScale, float InFrequency, bool InRight) const
{
    const Vec3F vel = GetRB().GetVelocity();
...
void ECS::PlayerAnimator::UpdateHands()
{
...
void ECS::PlayerAnimator::UpdateHead()
{
    const float dt = static_cast<float>(Utility::Time::Get().Delta());
...
void ECS::PlayerAnimator::SetHands(const HandState& InRight, const HandState& InLeft)
{
    targetRight = InRight;
    targetLeft = InLeft;
}

void ECS::PlayerAnimator::SetHead(const HeadState& InHead)
{
    targetHead = InHead;
}